function [flag] = jt_isprefpair(n,v,w,verb)
...
%       [2^n-1 1] M-sequence 1
%   w = [1 q]     Feedback Tap Positions 2
%       [2^n-1 1] M-sequence 2
%   
% OPTIONS
...
if length(v)~=2^n-1; v=jt_make_mls_code(n,v); end
if length(w)~=2^n-1; w=jt_make_mls_code(n,w); end
...
for q=qs
    for k=ks
        if (mod(n,2) == 1 && (q==2^k+1 || q==2^(2*k)-2^k+1) && gcd(n,k)==1) || ...
           (mod(n,4) == 2 && (q==2^k+1 || q==2^(2*k)-2^k+1) && gcd(n,k)==2)    
            K=cat(2,K,k);
            Q=cat(2,Q,q);
        end
    end
